  ...
  $("#stuemail").on("keypress blur", function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (data != 0) {
          $("#statusMsg2").html(
            '<small style="color:red;font-size:14px;padding-left:5px;"> Email ID Already Registered ! </small>'
          );
          $("#signup").attr("disabled", true);
        } else if (data == 0 && reg.test(stuemail)) {
          $("#statusMsg2").html(
            '<small style="color:green;font-size:14px;padding-left:5px;"> There you go ! </small>'
          );
          $("#signup").attr("disabled", false);
        } else if (!reg.test(stuemail)) {
          $("#statusMsg2").html(
            '<small style="color:red;font-size:14px;padding-left:5px;"> Please Enter Valid Email e.g. example@mail.com </small>'
          );
          $("#signup").attr("disabled", false);
        }
        if (stuemail == "") {
          $("#statusMsg2").html(
            '<small style="color:red;font-size:14px;padding-left:5px;"> Please Enter Email ! </small>'
          );
        }
      }
    });
...
function addStu() {
  ...
  if (stuname.trim() == "") {
    $("#statusMsg1").html(
      '<small style="color:red;font-size:14px;padding-left:5px;"> Please Enter Name ! </small>'
    );
    $("#stuname").focus();
    return false;
    ...
    $("#statusMsg3").html(
      '<small style="color:red;font-size:14px;padding-left:5px;"> Please Enter Password ! </small>'
    );
    $("#stupass").focus();
    return false;
  } else {
